...
class ViTill(nn.Module):
    ...
    def forward_with_attn(self, x):
        ...
        attn_list = []
        for i, blk in enumerate(self.decoder):
            x, attn = blk(x, return_attention=True, attn_mask=attn_mask)
            de_list.append(x)
...
class ASRB(nn.Module):
    ...
    def __init__(
        ...
        super(ASRB_Former, self).__init__()
        self.encoder = encoder
        self.bottleneck = bottleneck
        self.decoder = decoder
        self.target_layers = target_layers
    ...
    def forward_with_queries_attn(self, x):
        x = self.encoder.prepare_tokens(x)
        B, L, _ = x.shape
        en_list = []
        for i, blk in enumerate(self.encoder.blocks):
        ...
        attn_list = []
        for blk in self.bottleneck:
            queries, attn = blk(queries.unsqueeze(0).repeat((B, 1, 1)), x, return_attention=True)
            attn = torch.mean(attn, dim=1)  # 多个注意力头求平均
            attn = attn.reshape([x.shape[0], -1, side, side]).contiguous()
            attn_list.append(attn)
    ...
    def forward_with_attn(self, x):
        ...
        attn_list = []
        for i, blk in enumerate(self.decoder):
            x, attn = blk(x, queries, return_attention=True)
            attn = torch.mean(attn, dim=1)  # 多个注意力头求平均
            attn = attn.permute(0, 2, 1).reshape([x.shape[0], -1, side, side]).contiguous()
            attn_list.append(attn)
    ...
    def forward_features_list(self, x):
        ...
        if not self.remove_class_token:  # class tokens have not been removed above
            en_list = [e[:, 1 + self.num_register_tokens:, :] for e in en_list]
            de_list = [d[:, 1 + self.num_register_tokens:, :] for d in de_list]

        en_list = [e.permute(0, 2, 1).reshape([x.shape[0], -1, side, side]).contiguous() for e in en_list]
        de_list = [d.permute(0, 2, 1).reshape([x.shape[0], -1, side, side]).contiguous() for d in de_list]
